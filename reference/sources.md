# Sources

Every citation from the session, plus the sources behind this companion.

Where a link is given it has been checked and is verified in CI on every change.
Where no link is given, the citation is complete enough to find the document but
the canonical URL has not been verified, and it is marked so rather than guessed
at. A confident-looking link to the wrong page is worse than no link.

---

## Department policy and doctrine

**S1** — DAF launches plan to bolster AI workforce. af.mil, April 2026.
https://www.af.mil/News/Article-Display/Article/4472626/daf-launches-plan-to-bolster-ai-workforce/

**S2** — AI Ready by 2025, AI Competitive by 2027. MeriTalk.
*URL not verified.*

**S3** — DoD adopts 5 principles of artificial intelligence ethics. Army.mil,
February 2020. *URL not verified.*

**S4** — Responsible AI Strategy and Implementation Pathway. DoD, June 2022.
*URL not verified.*

**S5** — Software Fast Track initiative, DoD CIO. war.gov, 2025.
*URL not verified.*

**S6** — The DoD SWFT Initiative and the Promise of cATO Fulfilled. Anchore.
*URL not verified.*

**S9** — NSPM-11, Artificial Intelligence in the National Security Enterprise.
The White House, June 5, 2026.
https://www.whitehouse.gov/presidential-actions/2026/06/national-security-presidential-memorandum-nspm-11/

**S10** — Department of War AI strategy memoranda. January 9, 2026.
*URL not verified.*

**S11** — DAF AI Strategy. Cleared for open publication April 17, 2026.
*URL not verified.*

**S12** — DAF AI Hiring and Talent Development Plan, Total Force. April 2026.
*URL not verified.*

**S16** — Air Force Doctrine Note 25-1, Artificial Intelligence. Published
April 8, 2025.
https://www.doctrine.af.mil/Portals/61/documents/AFDN_25-1/AFDN%2025-1%20Artificial%20Intelligence.pdf

**S22** — Air Force Proficiency Code Key. Read verbatim from CFETP 2R1X1
(Maintenance Production Management) and CFETP 3S2X1 (Education and Training),
confirmed 26 August 2026. CFETPs are published at
https://www.e-publishing.af.mil/ — note that the static file host blocks
automated fetching, so browse rather than script it.

---

## Adoption evidence

**S8** — GenAI.mil at nearly 1.7M users and 100,000+ custom agents. DoD CDAO
Cameron Stanley, AWS Summit DC. Defense One, July 2, 2026. Official self-report,
not audited.
https://www.defenseone.com/technology/2026/07/genaimil-records-almost-17m-users-plans-new-model-additions/414569/

**S13** — The GenAI Divide: State of AI in Business 2025. MIT Project NANDA,
July 2025. Cross-sectional industry study. *URL not verified.*

Note on the widely quoted 95 percent failure figure from this study: it rests
largely on 52 qualitative interviews, and the authors themselves called it
directionally accurate. The failure *mode* is well supported. The number is not
load-bearing and is deliberately not used here.

**S14** — Over 40 percent of agentic AI projects will be canceled by end of 2027.
Gartner, June 25, 2025. Forecast. *URL not verified.*

**S15** — Artificial Intelligence Acquisitions. GAO-26-107859, April 13, 2026.
https://www.gao.gov/products/gao-26-107859

**S21** — PANDA, Predictive Analytics and Decision Assistant. Rapid Sustainment
Office, selected as an Air Force system of record. DefenseScoop, May 10, 2023.
https://defensescoop.com/2023/05/10/air-force-selects-ai-enabled-predictive-maintenance-program-as-system-of-record/

**S28** — The 10/20/70 rule: successful AI adoption is roughly 10 percent
algorithms, 20 percent technology and data, and 70 percent people and process.
Boston Consulting Group. *URL not verified.*

---

## Measurement

**S7** — Measuring the Impact of Early-2025 AI on Experienced Open-Source
Developer Productivity. METR, July 2025. Randomized controlled trial, 16
developers, 246 tasks.
https://arxiv.org/abs/2507.09089

**S27** — We are Changing our Developer Productivity Experiment Design. METR,
February 24, 2026. Why the study was redesigned rather than repeated.
https://metr.org/blog/2026-02-24-uplift-update/

**S25** — Work based assessment, and Miller's pyramid of clinical competence.
The *Knows / Knows how / Shows how / Does* progression.
https://pmc.ncbi.nlm.nih.gov/articles/PMC1125657/

**S26** — Entrustable Professional Activities and Entrustment Decision-Making in
Health Professions Education. Competencies are qualities of individuals; EPAs are
units of work.
https://www.ncbi.nlm.nih.gov/books/NBK619385/

---

## Autonomy, security and enforcement

**S17** — Autonomy and Agency in Agentic AI. Two-axis autonomy and agency design
space, five levels each, plus six architectural tactics. Safin and Balta,
fortiss, May 2026.
https://arxiv.org/abs/2605.12105

**S18** — OWASP Top 10 for Large Language Model Applications, 2025. Prompt
injection at number one; vector and embedding weaknesses added as their own
category.
https://owasp.org/www-project-top-10-for-large-language-model-applications/

**S19** — NIST AI Risk Management Framework, AI 100-1.
https://www.nist.gov/itl/ai-risk-management-framework
and NIST SP 800-53 Rev. 5.
https://csrc.nist.gov/pubs/sp/800/53/r5/upd1/final

**S20** — Customer-facing ordering bot completes a programming homework request
before taking the order. Media reporting, 2026. *URL not verified.*

**S23** — Large reasoning models are autonomous jailbreak agents. Overall
jailbreak success rate of 97.14 percent across model combinations. Nature
Communications, 2026.
https://www.nature.com/articles/s41467-026-69010-1

**S24** — Involuntary Jailbreak: On Self-Prompting Attacks. Success rates above
90 out of 100 attempts, with guardrails collapsing across several vendors'
leading models.
https://arxiv.org/abs/2508.13246

---

## The two learning paths

**Breadth.** Udacity for Government.
https://www.udacity.com/government/overview

**Depth.** Ascendient Learning.
https://www.ascendientlearning.com/it-training/topics/generative-ai
